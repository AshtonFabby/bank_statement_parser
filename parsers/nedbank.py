    ...
    def extract_transactions(self) -> pd.DataFrame:
        ...
        for page_text in self._iterate_pages():
            for line in page_text.split("\n"):
                ...
                # Parse amounts - Nedbank has Fees, Debits, Credits, Balance columns
                debit = 0.0
                credit = 0.0
                balance = clean_amount(amounts[-1])

                if len(amounts) >= 2:
                    non_balance_amounts = amounts[:-1]

                    for i, amt in enumerate(non_balance_amounts):
                        amt_val = clean_amount(amt)
                        if amt_val > 0:
                            if len(non_balance_amounts) == 1:
                                if "fee" in description.lower():
                                    debit = amt_val
                                else:
                                    amt_pos = line.find(amt)
                                    if amt_pos > len(line) * 0.5:
                                        credit = amt_val
                                    else:
                                        debit = amt_val
                            elif len(non_balance_amounts) >= 2:
                                if i == len(non_balance_amounts) - 1:
                                    credit = amt_val
                                elif i == len(non_balance_amounts) - 2:
                                    debit = amt_val

                # Use balance change as fallback
                if len(amounts) == 2 and debit == 0 and credit == 0:
